...
static String baseCss() {
  return
      ...
      "#langSeg{display:flex;gap:4px;justify-content:center;padding:6px 0;margin-top:6px}" \
      "#langSeg .langBtn{border:var(--linew) solid var(--line);background:#000;color:var(--fg);padding:4px 8px;font-size:10px;letter-spacing:.18em;text-transform:uppercase;font-weight:700;border-radius:0;min-width:44px;text-align:center;transition:background .16s ease,color .16s ease}" \
      "#langSeg .langBtn.active{background:var(--accent);color:#111}" \
      "#langSeg .langBtn:not(.active){background:#000}" \
      ...
      ".statusNote{font-size:13px;color:var(--mut);line-height:1.5}" \
        "footer{margin-top:32px;display:flex;flex-direction:column;align-items:center;gap:8px;font-size:11px;color:var(--mut);text-transform:uppercase;letter-spacing:.24em}";
}
...
// ================= HTML helpers =================
static String languageSwitcherHtml() {
  String s = "<div id='langSeg'>";
...
static String htmlHeaderLogin(const String &title) {
  ...
  s += "<div class='wrap'><main class='centerWrap'><div class='card cardCenter'>";
...
static String htmlFooter() {
  String s = "<footer>";
  s += languageSwitcherHtml();
  s += "<div class='kv'>SRKLink Control</div></footer></div></body></html>";
  return s;
}
...
function renderScan(d){
  ...
  const items = (d&&Array.isArray(d.networks))?d.networks:[];
  if(!items.length){
    list.innerHTML='<div class="item">'+text('none')+'</div>';
    const countLabel = (d&&typeof d.count==='number') ? text('scan_ready')+' '+d.count : text('none');
    scanStatus.textContent=countLabel;
    return;
  }
  items.forEach(ap=>{
  ...
  scanStatus.textContent=text('scan_ready')+' '+(d&&typeof d.count==='number'?d.count:items.length);
}
async function scanStart(){
  scanStatus.textContent=text('scan_wait');
  list.innerHTML='<div class="item">'+text('scan_wait')+'</div>';
  try{
    const res=await fetch('/scan?start=1');
...
static void handleScan() {
  ...
  if (server.hasArg("start")) {
    ...
    wifi_ps_type_t previousPs;
    if (esp_wifi_get_ps(&previousPs) != ESP_OK) {
      previousPs = WIFI_PS_MIN_MODEM;
    }
    ...
    esp_err_t startResult = esp_wifi_scan_start(&config, true /* block until complete */);
    ...
    uint16_t count = 0;
    esp_err_t countResult = esp_wifi_scan_get_ap_num(&count);
    ...
    for (uint16_t i = 0; i < count; ++i) {
      ...
      String ss = String(reinterpret_cast<const char *>(rec.ssid));
      json += "{\"ssid\":\"" + jsonEscape(ss) + "\",\"rssi\":" + String(rec.rssi) + ",\"enc\":\"" + encToStr(rec.authmode) + "\"}";
    }
    json += "]}";

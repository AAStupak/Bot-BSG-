...
static void handleScan() {
  ...
  if (server.hasArg("start")) {
    ...
    if (apCount > 0) {
      records = static_cast<wifi_ap_record_t *>(malloc(sizeof(wifi_ap_record_t) * apCount));
      if (!records) {
        WiFi.scanDelete();
        server.send(500, "application/json", "{\"error\":\"no_memory\"}");
        return;
      }
    ...
    String json = "{\"ok\":true,\"count\":" + String(apCount) + ",\"networks\":[";
    for (uint16_t i = 0; i < apCount; ++i) {
      if (i) json += ",";
      String ss = String(reinterpret_cast<const char *>(records[i].ssid));
      json += "{\"ssid\":\"" + jsonEscape(ss) + "\",\"rssi\":" + String(records[i].rssi) + ",\"enc\":\"" + encToStr(records[i].authmode) + "\"}";
    }
    json += "]}";
    if (records) {
      free(records);
